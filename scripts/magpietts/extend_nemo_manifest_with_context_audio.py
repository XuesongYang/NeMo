...
class EmbeddingSimilarityExtractorSharded(pl.LightningModule):
    ...
    def _process_and_flush_speakers_local(self):
        ...
        for speaker_id in speakers_to_process_now:
            ...
            # Using original indices as identifiers - much simpler and avoids floating-point precision issues
            # This prevents an item from being selected as its own context
...
def _get_parsed_speaker_id_for_dataset(dataset_name_arg, record):
    ...
    elif dataset_name_arg == "Emilia-YODAS":
        return _parse_speaker_id_nvyt2505(record)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10_000,
        help="Maximum size of context pool per speaker to prevent OOM. If a speaker has more items, a random sample will be used as context pool, but all items will still be processed.",
    )

...
class SharPredictionWriter(BasePredictionWriter):
    ...
    def write_on_batch_end(self, trainer, pl_module, prediction, batch_indices, batch, batch_idx, dataloader_idx):
        ...
        if trainer.global_rank == 0:
            for _pred_cuts, _context_recordings in gathered_objects:
                for cut, recording in zip(_pred_cuts, _context_recordings):
    ...
    def convert_prediction_to_cuts(self, prediction):
        ...
        target_audio_filepaths = prediction["target_audio_filepath"]
        target_codes = prediction["target_codes"]
        context_audio_filepaths = prediction["context_audio_filepath"]
        context_codes = prediction["context_codes"]
        speakers = prediction["speaker"]
        texts = prediction["text"]
        languages = prediction["language"]
        ...
        for idx, target_audio_filepath in enumerate(target_audio_filepaths):
            ...
            # supervision of target recording
            supervision = SupervisionSegment(
                ...
                custom={key: val[idx] for key, val in meta_fields.items()} if not meta_fields else None,
            )

            cut = MonoCut(
                id=f"cut-{target_recording_id}",
                start=0.0,
                duration=target_duration,
